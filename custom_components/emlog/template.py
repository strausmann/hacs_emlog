...
class EmlogAdvanceDifferenceSensor(SensorEntity):
    ...
    def native_value(self) -> float | None:
        ...
        try:
            ...
            if not yearly_cost_state or yearly_cost_state.state in ("unknown", "unavailable"):
                return None
            ...
            if helper_id:
                ...
                if state and state.state not in ("unknown", "unavailable"):
                    ...
                    monthly_advance = float(self._entry.options.get(config_key, self._entry.data.get(config_key, default)))
            else:
                monthly_advance = float(self._entry.options.get(config_key, self._entry.data.get(config_key, default)))
